...
class LiveController extends Controller
{
    ...
    public function live(Request $request) {
        ...
        $id = $request->input('id');
        $ch = curl_init();
        $url = "http://www.liaogou168.com/aik/lives/detailJson/$id" . '.json';
        ...
        return view('pc.live.bj', $result);
    }

    public function bj(Request $request){
        broadcast(new ChatMessageWasReceived($request->input('msg'),$request->input('name')));
        return response()->json(['result' => 'ok'], 200);
    }

    public function bj2(Request $request){
        broadcast(new SendMessage($request->input('msg')));
        return response()->json(['result' => 'ok'], 200);
    }
